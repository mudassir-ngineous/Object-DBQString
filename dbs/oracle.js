var insertQueryMaker = function(jsonObj){
...
var updateQueryMaker = function(jsonObj){
	if(jsonObj.hasOwnProperty("table")){
...
var deleteQueryMaker = function(jsonObj){
	if(jsonObj.hasOwnProperty("table")){
		var setData = jsonObj.set;
		var fieldUpdateStr = "";
		var conditionStr = "";
...
var selectQueryMaker = function(jsonObj){
	if(jsonObj.hasOwnProperty("table") && jsonObj.hasOwnProperty("columns")){
		...
		if(typeof(jsonObj.table) === 'object' && jsonObj.table.length>1){
			if(jsonObj.distinct){
				finalStr = "SELECT DISTINCT " + fieldStr + " FROM " + jsonObj.table.join(",") + conditionStr;
			}else{
			...
			if(jsonObj.distinct){
				finalStr = "SELECT DISTINCT " + fieldStr + " FROM " + jsonObj.table + conditionStr;
			}else{
...
var getFormattedString = function(s){
	if(typeof(s) === "number"){
		return s;
...
var processConditionObj = function(cond_obj,operator){
	var where_list=[];
...
var obj = {
	"insert":insertQueryMaker,
	"update":updateQueryMaker,
	"delete":deleteQueryMaker,
	"select":selectQueryMaker
};
